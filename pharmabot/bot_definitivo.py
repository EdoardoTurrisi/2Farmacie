...
def start(update, context):
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text=f"Ciao {user.first_name}, benvenuto su Pharmabot! Con questo bot potrai ottenere informazioni riguardo alle farmacie nella tua zona (/elencofarmacie) e a quella più vicina a te (/geolocalizzazione). \nSe trovi alcuni problemi con il bot non esitare a scriverci all'email edoardogiuseppeturrisi@gmail.com")
    ...
    context.bot.send_message(chat_id=update.effective_chat.id, text="Per favore condividi la tua posizione con il bottone apposito prima di eseguire il comando della geolocalizzazione.", reply_markup=reply_markup)
...
def nearest_pharmacy(update, context):
    ...
    # 
    results = []
    cursor.execute("select * from bussolengo union select * from castelnuovo union select * from mozzecane union select * from pastrengo union select * from pescantina union select * from sommacampagna union select * from sona union select * from valeggio union select * from vigasio union select * from villafranca")
    addresses = [address[0] for address in cursor.execute("SELECT Indirizzo FROM {}".format(table_name))]
    distances = []
    for address in addresses:
        geolocator = Nominatim(user_agent="pharmabot")
